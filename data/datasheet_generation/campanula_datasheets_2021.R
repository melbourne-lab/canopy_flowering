# For generating campanula datasheets in 2021

library(dplyr)
library(tidyr)

### Jul 19

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  arrange(Plot, Tag) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_19-07-21.csv',
            na = '', row.names = FALSE)

read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_19-07-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  write.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv', row.names = FALSE, na = '')

### Jul 20

plots.0720 = read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv')) %>%
  filter(Date %in% '7/19/21') %>%
  group_by(Plot) %>%
  summarise(nfl = sum(Fl_open))

set.seed(9900)

sample(plots.0720$Plot[plots.0720$nfl > 0])
# [1] 74 17 37 71 53  7  6 73
sample(plots.0720$Plot[!plots.0720$nfl])
# [1] 26 82 67
sample(plots.0720$Plot[plots.0720$nfl > 0], size = 4)
# [1] 53 73 71  6

### July 22 2021

# get phen datasheet
read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv')) %>%
  arrange(Plot, Tag) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_22-07-21.csv',
            na = '', row.names = FALSE)

# get data entry csv

read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_22-07-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv', row.names = FALSE, na = '')

### Jul 26 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one', Note)) %>%
  # (at some point - next time - will want to remove plants with more than 3 consecutive
  # empty records)
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_26-07-21.csv',
            na = '', row.names = FALSE)

read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_26-07-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv', row.names = FALSE, na = '')

### Jul 27 2021

plots.0727 = read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv')) %>%
  filter(Date %in% '7/26/21') %>%
  group_by(Plot) %>%
  summarise(nfl = sum(Fl_open))

set.seed(1516)

sample(plots.0727$Plot, 15)
#  [1]  7 45  1 10 53 67 17  3 82  8 34 37 74 66  2

### July 29 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # (at some point - next time - will want to remove plants with more than 3 consecutive
  # empty records)
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_29-07-21.csv',
            na = '', row.names = FALSE)

read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_29-07-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv', row.names = FALSE, na = '')

### August 2 2021

plots.0729 = read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv')) %>%
  filter(Date %in% '7/29/21') %>%
  group_by(Plot) %>%
  summarise(nfl = sum(Fl_open))

set.seed(90208223)

sample(plots.0729$Plot, 15)
#  [1] 10 57 66 17  6 74 58 73 45 37 79 70  3  7 38

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # (at some point - next time - will want to remove plants with more than 3 consecutive
  # empty records)
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_02-08-21.csv',
            na = '', row.names = FALSE)

read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_02-08-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv', row.names = FALSE, na = '')

### Aug 5 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # (at some point - next time - will want to remove plants with more than 3 consecutive
  # empty records)
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_05-08-21.csv',
            na = '', row.names = FALSE)
  
# data entry
read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_05-08-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv', row.names = FALSE, na = '')

### Aug 9 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # Remove plants with more than two records listed as "can't find"
  #   (possibly redundant due to below)
  filter(sum(grepl("can't find", Note)) <= 1) %>%
  # Remove plants with more than two records with number of stems NA
  filter(sum(is.na(Fl_stems)) <= 1) %>%
   # Get dates (for use as "done')
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note, Prev_date = Date) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_date, Prev_note) %>% 
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_09-08-21.csv',
            na = '', row.names = FALSE)

# csv for data entry
read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_09-08-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_09-08-21.csv', row.names = FALSE, na = '')

# Pollinator observations:
plots.0809 = read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv')) %>%
  filter(Date %in% '8/5/21') %>%
  group_by(Plot) %>%
  summarise(nfl = sum(Fl_open, na.rm = TRUE))

set.seed(55)

sample(plots.0809$Plot, 18)

### Aug 12 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-08-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # Remove plants with more than two records listed as "can't find"
  #   (possibly redundant due to below)
  filter(sum(grepl("can't find", Note)) <= 1) %>%
  # Remove plants with more than two records with number of stems NA
  filter(sum(is.na(Fl_stems)) <= 1) %>%
  # Get dates (for use as "done')
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  # For some reason - some old plants are sticking around despite other forms of post-processing
  # remove them here - get rid of plants w/ no records in the last tne days
  filter(max(Date) > as.Date('08/2/21', format = '%m/%d/%y')) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note, Prev_date = Date) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_date, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_12-08-21.csv',
            na = '', row.names = FALSE)

# file for data entry
read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_12-08-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_12-08-21.csv', row.names = FALSE, na = '')

### Aug 16 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_12-08-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # Remove plants with more than two records listed as "can't find"
  #   (possibly redundant due to below)
  filter(sum(grepl("can't find", Note)) <= 1) %>%
  # Remove plants with more than two records with number of stems NA
  filter(sum(is.na(Fl_stems)) <= 1) %>%
  # Get dates (for use as "done')
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  # For some reason - some old plants are sticking around despite other forms of post-processing
  # remove them here - get rid of plants w/ no records in the last tne days
  filter(max(Date) > as.Date('08/5/21', format = '%m/%d/%y')) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note, Prev_date = Date) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_date, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_16-08-21.csv',
            na = '', row.names = FALSE)

# data entry
read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_16-08-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_16-08-21.csv', row.names = FALSE, na = '')

# pollinators
plots.0816 = read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_12-08-21.csv')) %>%
  filter(Date %in% '8/12/21', Fl_open > 0 | grepl('pv', Note)) %>%
  group_by(Plot) %>%
  summarise(nfl = sum(Fl_open, na.rm = TRUE))

set.seed(125)

sample(plots.0816$Plot, 18)

### Aug 20 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_12-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_16-08-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # Remove plants with more than two records listed as "can't find"
  #   (possibly redundant due to below)
  filter(sum(grepl("can't find", Note)) <= 1) %>%
  # Remove plants with more than two records with number of stems NA
  filter(sum(is.na(Fl_stems)) <= 1) %>%
  # Get dates (for use as "done')
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  # For some reason - some old plants are sticking around despite other forms of post-processing
  # remove them here - get rid of plants w/ no records in the last tne days
  filter(max(Date) > as.Date('08/9/21', format = '%m/%d/%y')) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note, Prev_date = Date) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_date, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_20-08-21.csv',
            na = '', row.names = FALSE)

# data entry
read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_20-08-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_20-08-21.csv', row.names = FALSE, na = '')

### Aug 23 3021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_12-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_16-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_20-08-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # Remove plants with more than two records listed as "can't find"
  #   (possibly redundant due to below)
  filter(sum(grepl("can't find", Note)) <= 1) %>%
  # Remove plants with more than two records with number of stems NA
  filter(sum(is.na(Fl_stems)) <= 1) %>%
  # Get dates (for use as "done')
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  # For some reason - some old plants are sticking around despite other forms of post-processing
  # remove them here - get rid of plants w/ no records in the last tne days
  filter(max(Date) > as.Date('08/12/21', format = '%m/%d/%y')) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note, Prev_date = Date) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_date, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_23-08-21.csv',
            na = '', row.names = FALSE)

read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_23-08-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_23-08-21.csv', row.names = FALSE, na = '')

# pollinators
plots.0823 = read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_20-08-21.csv')) %>%
  filter(Date %in% '8/20/21', Fl_open > 0 | grepl('pv', Note)) %>%
  group_by(Plot) %>%
  summarise(nfl = sum(Fl_open, na.rm = TRUE))

set.seed(908)

sample(plots.0823$Plot)

### Aug 26 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_12-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_16-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_20-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_23-08-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # Remove plants with more than two records listed as "can't find"
  #   (possibly redundant due to below)
  filter(sum(grepl("can't find", Note)) <= 1) %>%
  # Remove plants with more than two records with number of stems NA
  filter(sum(is.na(Fl_stems)) <= 1) %>%
  # Get dates (for use as "done')
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  # For some reason - some old plants are sticking around despite other forms of post-processing
  # remove them here - get rid of plants w/ no records in the last tne days
  filter(max(Date) > as.Date('08/15/21', format = '%m/%d/%y')) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note, Prev_date = Date) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_date, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_26-08-21.csv',
            na = '', row.names = FALSE)


read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_26-08-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_26-08-21.csv', row.names = FALSE, na = '')

### Aug 31 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_12-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_16-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_20-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_23-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-08-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # Remove plants with more than two records listed as "can't find"
  #   (possibly redundant due to below)
  filter(sum(grepl("can't find", Note)) <= 1) %>%
  # Remove plants with more than two records with number of stems NA
  filter(sum(is.na(Fl_stems)) <= 1) %>%
  # Get dates (for use as "done')
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  # For some reason - some old plants are sticking around despite other forms of post-processing
  # remove them here - get rid of plants w/ no records in the last tne days
  filter(max(Date) > as.Date('08/19/21', format = '%m/%d/%y')) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note, Prev_date = Date) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_date, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_31-08-21.csv',
            na = '', row.names = FALSE)


read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_31-08-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_31-08-21.csv', row.names = FALSE, na = '')

# pollinators
plots.0831 = read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_26-08-21.csv')) %>%
  filter(Date %in% '8/26/21', Fl_open > 0 | grepl('pv', Note)) %>%
  group_by(Plot) %>%
  summarise(nfl = sum(Fl_open, na.rm = TRUE))

set.seed(220)

sample(plots.0831$Plot)
# [1] 10 17 55 29 74 72 21 16

### Sep 4 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_12-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_16-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_20-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_23-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_31-08-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # Remove plants with more than two records listed as "can't find"
  #   (possibly redundant due to below)
  filter(sum(grepl("can't find", Note)) <= 1) %>%
  # Remove plants with more than two records with number of stems NA
  filter(sum(is.na(Fl_stems)) <= 1) %>%
  # Get dates (for use as "done')
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  # For some reason - some old plants are sticking around despite other forms of post-processing
  # remove them here - get rid of plants w/ no records in the last tne days
  filter(max(Date) > as.Date('08/22/21', format = '%m/%d/%y')) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note, Prev_date = Date) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_date, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_04-09-21.csv',
            na = '', row.names = FALSE)

read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_04-09-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_04-09-21.csv', row.names = FALSE, na = '')

### Sep 9 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_12-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_16-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_20-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_23-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_31-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_04-09-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # Remove plants with more than two records listed as "can't find"
  #   (possibly redundant due to below)
  filter(sum(grepl("can't find", Note)) <= 1) %>%
  # Remove plants with more than two records with number of stems NA
  filter(sum(is.na(Fl_stems)) <= 1) %>%
  # Get dates (for use as "done')
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  # For some reason - some old plants are sticking around despite other forms of post-processing
  # remove them here - get rid of plants w/ no records in the last tne days
  filter(max(Date) > as.Date('08/25/21', format = '%m/%d/%y')) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note, Prev_date = Date) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_date, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_09-09-21.csv',
            na = '', row.names = FALSE)

read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_09-09-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_09-09-21.csv', row.names = FALSE, na = '')

### Sep 14 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_12-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_16-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_20-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_23-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_31-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_04-09-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-09-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # Remove plants with more than two records listed as "can't find"
  #   (possibly redundant due to below)
  filter(sum(grepl("can't find", Note)) <= 1) %>%
  # Remove plants with more than two records with number of stems NA
  filter(sum(is.na(Fl_stems)) <= 1) %>%
  # Get dates (for use as "done')
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  # For some reason - some old plants are sticking around despite other forms of post-processing
  # remove them here - get rid of plants w/ no records in the last tne days
  filter(max(Date) > as.Date('08/28/21', format = '%m/%d/%y')) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note, Prev_date = Date) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_date, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_14-09-21.csv',
            na = '', row.names = FALSE)

read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_14-09-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_14-09-21.csv', row.names = FALSE, na = '')

### Sep 23 2021

read.csv('data/raw_data/data_2021/Campanula_newplants_2021.csv') %>%
  rbind(read.csv('data/raw_data/data_2021/campa_entry_19-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_22-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_29-07-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_02-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_05-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_12-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_16-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_20-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_23-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_26-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_31-08-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_04-09-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_09-09-21.csv'),
        read.csv('data/raw_data/data_2021/campa_entry_14-09-21.csv')) %>%
  arrange(Plot, Tag, desc(Date)) %>%
  # Get rid of gone or collected plants
  group_by(Tag) %>%
  filter(!grepl('[Cc]ollected|[Gg]one|[Ii]ggy', Note)) %>%
  # Remove plants with more than two records listed as "can't find"
  #   (possibly redundant due to below)
  filter(sum(grepl("can't find", Note)) <= 1) %>%
  # Remove plants with more than two records with number of stems NA
  filter(sum(is.na(Fl_stems)) <= 1) %>%
  # Get dates (for use as "done')
  mutate(Date = as.Date(Date, format = '%m/%d/%y')) %>%
  # Pick out plants not finished yet
  #   finished = no open flowers in two most recent records, number of done flowers same
  #   and no yv in notes
  #   (requires sorting rows by date)
  arrange(Plot, Tag, desc(Date)) %>%
  group_by(Plot, Tag) %>%
  # Mark finished plants
  mutate(done = (!Fl_open[1] & !Fl_open[2]) & 
           (Fl_done[1] <= Fl_done[2]) & 
           !(grepl('pv|bud', Note[1]) | grepl('pv|bud', Note[2])),
         done = ifelse(is.na(done), FALSE, done),
         done = ifelse(done, 'done', NA)) %>%
  # For some reason - some old plants are sticking around despite other forms of post-processing
  # remove them here - get rid of plants w/ no records in the last tne days
  filter(max(Date) > as.Date('9/1/21', format = '%m/%d/%y')) %>%
  ungroup() %>%
  # Get only one row per plant
  distinct(Tag, .keep_all = TRUE) %>%
  filter(is.na(done) | !(done %in% 'done')) %>%
  mutate(Prev = paste(Fl_stems, Fl_open, Fl_done, sep = ';')) %>%
  rename(Prev_note = Note, Prev_date = Date) %>%
  mutate(Date = NA, Fl_stems = NA, Fl_open = NA, Fl_done = NA, Q = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Prev, Prev_date, Prev_note) %>%
  write.csv(file = 'data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_23-09-21.csv',
            na = '', row.names = FALSE)

read.csv('data/datasheet_generation/datasheet_outputs/data_datasheets_2021/campa_23-09-21.csv') %>%
  mutate(Date = NA, Page = NA, Note = NA) %>%
  select(Date, Plot, Tag, Fl_stems, Fl_open, Fl_done, Q, Page, Note) %>%
  distinct(Tag, .keep_all = TRUE) %>% ## comment out in successive versions
  write.csv('data/raw_data/data_2021/campa_entry_23-09-21.csv', row.names = FALSE, na = '')
