...
# - Need to add the remaining October observations
...
this.plot.data = paste0(sensor.dir, grep('plot75\\_', dir(sensor.dir), value = TRUE)) %>%
  grep(pattern = 'raw', x = ., value = TRUE) %>%
  lapply(function(x) read.csv(x) %>% mutate(fn = x)) %>%
  do.call(what = rbind) %>%
  # Get only pyranometer readings
...
# Ah this is the one that got knocked over a lot

this.plot.data = this.plot.data %>%
  # Remove July 13 - 14
  filter(!(Month %in% 7 & Day %in% 13:14)) %>%
  # Remove July 23 - 26
  filter(!(Month %in% 7 & Day %in% 24:26))

# some other times that look not great, but this honestly could just be cloud
...
write.csv(this.plot.data,
          file = 'sensors_and_loggers/processed_data/plot75_pyran_clearsky_2021.csv',
          row.names = FALSE)
...
this.plot.data = paste0(sensor.dir, grep('plot77\\_', dir(sensor.dir), value = TRUE)) %>%
  grep(pattern = 'raw', x = ., value = TRUE) %>%
  lapply(function(x) read.csv(x) %>% mutate(fn = x)) %>%
  do.call(what = rbind) %>%
  # Get only pyranometer readings
